...
import AddReview from '../Accounts/AddReview';

const OrderDetails = () => {
   const location = useLocation();
   const {userProfile,firstName, lastName} = location.state.user;
   console.log(userProfile)
   const {_id,date,paidStatus,product,transactionId} = location.state.order;
   const {mobile, address, amount, currency, products} = product;
    ...
    return (
      <div className='container mx-auto'>
        ...
        <div className='w-full flex justify-center mt-9 mb-9'>   
            ...
            <p className="font-semibold text-gray-700 dark:text-gray-400">Customer Name: <span className='text-sm font-normal'>{firstName +' '+ lastName}</span></p>
        ...
        <div className="md:flex mx-auto mt-8 mb-10">
              ...
              <table className="table">
                ...
                <thead >
                  <tr>
                    ...
                    <th className='text-base font-bold'>Review</th>
                  </tr>
                </thead>
                <tbody>
                  {/* row 1 */}
                  {products.map(ct => <tr key={ct._id}>
                    ...
                    <div className="flex items-center justify-center md:w-1/4">
                      <span className="text-lg ml-9 text-center">{ct.quantity}</span>
                      </div>
                    </td>
                    <th>
                      <button className="btn btn-ghost btn-xs">${parseInt(ct.price * ct.quantity).toFixed(2)}</button>
                    </th>
                    <th><AddReview firstName={firstName} lastName={lastName} product={ct} userProfile={userProfile}></AddReview></th>
                  </tr>)}
          ...
          <div className='flex flex-col mx-auto bg-blue-600 px-5'>
            ...
            <p className='py-1 text-white flex flex-row justify-between'><span >Total Price:</span> ${calculateTotalPrice(products).toFixed(2)}</p>
